...
We construct three measures of cultural similarity using the CEPII gravity dataset's country-pair language indicators. The first measure, Same Nationality, equals one if both teammates represent the same country and zero otherwise. The second measure, Same Language, equals one if both teammates share an official national language (CEPII's \textit{comlang\_off}) and zero otherwise. \textcolor{red}{Linguistic proximity
is a continuous 0–1 index of ethnolinguistic closeness (higher = more similar language family).} The third measure, Language Proximity, equals one if both teammates share a widely-spoken ethnic language and zero otherwise (CEPII's \textit{comlang\_ethno}, defined as a language spoken as a mother tongue by at least 9\% of the population in both countries).\footnote{Monaco is entirely absent from the raw CEPII gravity table's language variables. We manually set Same Language and Language Proximity to 1 for any Monaco pairing against another French-official-language country (France, Belgium, Switzerland, Canada, Luxembourg). This affects 39 of the raw match observations.}


These measures reflect increasingly broad notions of cultural similarity. Shared nationality captures common institutional and cultural background; shared language more directly captures communication potential; and linguistic proximity recognizes that communication costs may also be lower between speakers of closely related languages even when they do not share the same official language.

Table 2: \textcolor{red}{I would remove Panel B and add the absolute number of partnerships next to the share in Panel A, 42.2\%=X teams for each of the 3 measure.}  In our sample,  \textcolor{red}{42.2\%} of teams consist of players sharing the same nationality, \textcolor{red}{56.0\%} share an official language, and the mean linguistic proximity is \textcolor{red}{0.568} on the 0--1 scale.
...
\multirow{4}{*}{Same language (AME)}
 ...
 & & & (0.081709) & (0.082578) \\
\addlinespace
\multirow{4}{*}{Ling.\ proximity (OLS coef.)}
 & own\_rank & 0.000139*** & & 0.000104** \\
 & & (0.000053) & & (0.000047) \\
 & composition & & 1.152*** & 1.145*** \\
 & & & (0.075655) & (0.076295) \\
\midrule
