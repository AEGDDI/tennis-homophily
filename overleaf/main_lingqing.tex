...
\textcolor{red}{We construct three measures of cultural similarity, all binary, using the CEPII gravity dataset's country-pair language indicators. The first measure, Same Nationality, equals one if both teammates represent the same country and zero otherwise. The second measure, Same Language, equals one if both teammates share an official national language (CEPII's \textit{comlang\_off}) and zero otherwise. The third measure, Language Proximity, equals one if both teammates share a widely-spoken ethnic language and zero otherwise (CEPII's \textit{comlang\_ethno}, defined as a language spoken as a mother tongue by at least 9\% of the population in both countries).\footnote{Monaco is not covered by the raw CEPII gravity table. We manually set Same Language and Language Proximity to 1 for any Monaco pairing against another French-official-language country (France, Belgium, Switzerland, Canada, Luxembourg), reflecting that French is Monaco's sole official language. This affects 39 of the raw match observations.}}
