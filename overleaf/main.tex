...
Organizational research suggests that effective coordination depends not only on communication itself but also on the emergence of shared mental models—compatible understandings of the task, the environment, and teammates’ likely actions that allow individuals to anticipate one another without constant explicit communication \citep{cannonbowers1993,klimoski1994,mathieu2000}. On the other hand, cultural similarity may facilitate the development of such shared understandings by reducing communication frictions and providing teammates with more compatible expectations about one another’s behaviour. Recent evidence shows that cultural similarity is associated with greater collaboration between professional football players \citep{bekesottaviano2025}. Yet despite extensive evidence on how culture influences communication and collaboration within organizations, comparatively little is known about whether cultural similarity ultimately translates into superior team performance. \added{Recent work also documents cultural sorting in team interactions. Békés and Ottaviano (2025), for example, show that culturally similar players are more likely to collaborate in professional football. We find a similar pattern in professional doubles tennis, where partnerships are substantially more likely to connect culturally similar players than would be expected under random matching. This naturally raises a second question: once partnerships are formed, does cultural similarity also translate into differences in team performance? We address this question first and subsequently return to partnership formation to examine how cultural sorting operates in our setting.}

In this paper, we study whether cultural similarity is associated with performance of small teams. We use professional men's Grand Slam doubles tennis as an empirical setup of organizational coordination, where two players with distinct individual abilities must repeatedly coordinate their actions to produce a joint outcome.  Many organizations rely on dyadic or small-team collaboration, including project teams, surgical teams, military units, emergency response teams, and entrepreneurial partnerships. \added{In project teams, teammates repeatedly exchange information, adjust responsibilities, and anticipate one another's actions. In surgical teams, coordination occurs through repeated communication and rapid adjustment under time pressure. In military teams, shared expectations and communication facilitate coordinated action when explicit communication is costly or constrained. Doubles tennis has the same basic organizational structure at a much smaller scale: two interdependent individuals repeatedly make decisions whose value depends partly on anticipating and adapting to the partner's actions.} Although these settings differ substantially in purpose and context, they share common organizational challenges. %team members must repeatedly coordinate their actions under time constraints while adapting to rapidly changing environments. 
...
\begin{tabular}{lc}
\toprule
\multicolumn{2}{l}{\textbf{Panel A: Cultural Similarity Shares (N = 2,101 partnerships)}} \\[2pt]
\toprule
Measure of cultural similarity& Share \\
\midrule
Same nationality  & 42.2\% \\
Same official language  & 55.9\% \\
Language proximity (0--1 scale) & 0.567 \\
\bottomrule
\end{tabular}
\hspace{1cm}
\begin{tabular}{lc}
\toprule
\multicolumn{2}{l}{\textbf{Panel B: Unique Teams (Partnerships) by Tournament}} \\[2pt]
\toprule
Tournament & Teams \\
\midrule
Australian Open & 542 \\
...
\begin{tablenotes}
\small
\item Note: Panel A reports shares over the 2,101 distinct tournament$\times$partnership pairings with complete ranking data (same rank-complete sample as Panel B and Section~\ref{sec:sorting}'s Table~\ref{tab:t7} ``All'' row) -- a pair advancing through several rounds of the same event is counted once, not once per match played. Panel B counts the same 2,101 pairings, broken down by tournament.
\end{tablenotes}
\end{threeparttable}
\end{table}
\FloatBarrier

In our sample,  \textcolor{red}{42.2\%} of teams consist of players sharing the same nationality, \textcolor{red}{55.9\%} share an official language, and the mean linguistic proximity is \textcolor{red}{0.567} on the 0--1 scale.
...
\added{As a check on the non-independence of the two team-rows within a match (a win for one team mechanically implies a loss for the other), we re-estimate Table \ref{tab:t3} keeping only one randomly-selected team per match (N=1,876), with robust standard errors and no clustering (since each match now contributes exactly one row). Repeated over 10 random seeds: the same-nationality AME averages 0.0312 (range 0.0024--0.0764, significant at the 5\% level in 2 of 10 draws), same-language averages 0.0392 (range 0.0146--0.0747, significant in 5 of 10 draws), and language proximity averages 0.0357 (range 0.0084--0.0772, significant in 4 of 10 draws) -- consistently the same sign and a broadly similar magnitude to Table \ref{tab:t3}, but genuinely not identical to it draw by draw. Halving the sample inflates standard errors by roughly $\sqrt{2}$, which is enough to push several individual draws below conventional significance, most visibly for same nationality (already the weakest of the three measures at baseline). We also verified that dropping the model's intercept in favor of full-rank tournament$\times$year fixed effects reproduces Table \ref{tab:t3} exactly (identical AMEs, standard errors, and log-likelihood), confirming the clustered, both-rows specification used throughout the paper -- the standard approach for this kind of paired, mechanically-linked data -- is not manufacturing the result.}
...
Table \ref{tab:t5} reports the results. The coefficients are remarkably stable across playing environments, suggesting that the result is not driven by any specific physical environment of the tournament. \added{The notes below every table (Tables \ref{tab:t3}--\ref{tab:t6b}) have been checked against the underlying estimation code -- controls, fixed effects, clustering, sample sizes, and robustness-spec descriptions all match what was actually estimated.}
...
\begin{tablenotes}
\small
\item *** p$<$0.01, ** p$<$0.05, * p$<$0.10. Standard errors in parentheses, clustered by match. \textit{exp\_mean\_dm} is demeaned at the sample mean (11.90 years, SD 5.17), so the Culture (C) row is the AME at mean team experience. An alternative specification adding the quadratic interaction C $\times$ exp\_mean\_dm$^2$ leaves this conclusion unchanged: the quadratic term is small and insignificant for all three measures (p$>$0.50), while the linear interaction for shared language and linguistic proximity remains positive and significant (p$\approx$0.04).
\end{tablenotes}
\end{threeparttable}
\end{table}
\FloatBarrier
...
We regress each cultural-similarity measure on the focal player's own doubles ranking, including tournament-by-year fixed effects. \added{Because the cultural composition of the player pool differs across tournament-years, a player's probability of having a culturally similar partner may vary mechanically with the set of players entering the event. We therefore extend the specification to also account for the cultural composition of the tournament field: for each focal player, the share of every \emph{other} player in that tournament-year's full field (every entrant, not only the rank-complete partnerships used above) who shares the focal player's nationality -- and, analogously, the mean pairwise value of \textit{same\_language}/\textit{ling\_prox} against every other player in the field -- matched to the corresponding outcome.} The specification is deliberately parsimonious: the purpose is not to explain partner selection comprehensively, but to test whether a player's own ranking predicts the cultural similarity of the partner within a given tournament-year, and whether that association survives controlling for own nationality and field composition.

\added{We estimate four specifications: (1) own ranking alone (as above); (2) own nationality (country fixed effects) in place of ranking; (3) own ranking plus the outcome-matched field-composition variable; and (4) own nationality plus field composition. Table~\ref{tab:t8} reports Specifications 1 and 3, which are estimable for all three outcomes.}

\begin{table}[htbp]
\color{blue}
\centering
\caption{Own Ranking, Tournament-Field Composition, and Partner-Culture Similarity \label{tab:t8}}
\begin{threeparttable}
\begin{tabular}{lccc}
\toprule
Outcome & Spec 1: own\_rank & Spec 3: own\_rank & Spec 3: field composition \\
\midrule
Same nationality (AME) & 0.000271*** & 0.000118** & 3.753*** \\
 & (0.000055) & (0.000053) & (0.253920) \\
Same language (AME) & 0.000148** & 0.000117** & 1.272*** \\
 & (0.000058) & (0.000053) & (0.082017) \\
Ling.\ proximity (OLS coef.) & 0.000141*** & 0.000105** & 1.149*** \\
 & (0.000053) & (0.000047) & (0.076017) \\
\midrule
$N$ (ego-rows) & 4,202 & 4,202 & 4,202 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item *** p$<$0.01, ** p$<$0.05, * p$<$0.10. Standard errors in parentheses, clustered by partnership. Both specifications include tournament$\times$year FE. Field composition is (\# other field entrants sharing the focal player's nationality)/(field size $-1$) for same nationality, and the analogous mean pairwise value for same language / linguistic proximity, computed against every other player entering that tournament-year (not only the rank-complete partnerships in the estimation sample).
\end{tablenotes}
\end{threeparttable}
\end{table}

The results in Table~\ref{tab:t8} confirm and extend the simple own-ranking result. In Spec 1, the coefficient on own ranking is positive and statistically significant for all three measures: 0.000271 for same nationality (p$<$0.001), 0.000148 for same language (p=0.011), and 0.000141 for linguistic proximity (p=0.008). Because a lower doubles-ranking number indicates a stronger player, the positive coefficient means that worse-ranked players are, if anything, more likely to have culturally similar partners. Adding field composition (Spec 3) shrinks the own-ranking coefficients by roughly half but leaves them positive and statistically significant for all three outcomes (p$<$0.05 throughout), while the composition variable itself is a strong, highly significant predictor in every case, as expected -- a player drawn from a tournament field with more compatriots or linguistically similar entrants is mechanically more likely to end up with a culturally similar partner. Own ranking therefore predicts partner similarity \emph{in addition to}, not merely \emph{because of}, the cultural composition of the field a player happens to be drawn from.

\added{Specifications 2 and 4 (own nationality replacing own ranking, with and without field composition) are estimable only for linguistic proximity, the one continuous outcome: the field-composition coefficient remains positive and significant (0.941, p$<$0.001 in Spec 4), consistent with Spec 3. For the two binary outcomes (same nationality, same language), the country-fixed-effects logit specifications fail to converge -- a quasi-complete-separation problem, not an estimation error -- because several of the 64 nationalities in the ego-row sample have very few observations (14 have fewer than 10, down to single players), so a given nationality's fixed effect can perfectly predict that player's own-nationality-partnering outcome. Pooling nationalities with fewer than 10 observations into a single category (51 remaining groups) does not resolve this for the binary outcomes. We therefore report Specifications 2 and 4 only for linguistic proximity; the finding that own ranking predicts partner similarity net of field composition (Spec 3) is the paper's main extension of this result.}
...
\added{The relatively flat trend around the Tokyo cycle in particular should not be read as evidence about whether players strategically build Grand Slam partnerships in anticipation of the Olympics, because the two populations of teams are largely distinct. Each Olympic doubles draw fields only 32 teams -- 22 nations at Tokyo 2021 and 23 at Paris 2024 -- and cross-referencing these 61 distinct Olympic pairings against every Grand Slam partnership in our 2018--2025 sample shows that only 16 (26\%; 9 of 32 at Tokyo, 10 of 32 at Paris) ever play together at a Grand Slam at all. This is not incidental: Olympic doubles eligibility does not require an existing partnership. A player ranked in the ATP doubles top 10 may enter with any compatriot ranked in the top 300 of either singles or doubles, and additional places are allocated by a combined ranking (each player's best of singles or doubles) that prioritizes pairs already accepted into the singles draw -- the pathway through which, for example, Rafael Nadal and Carlos Alcaraz partnered at the Paris Games despite never having played tour doubles together. Because most Olympic teams are assembled from outside the pool of regular Grand Slam doubles partnerships in the first place, the absence of a shift in Grand Slam partnership formation around the Games is more directly explained by this institutional separation between the two competitive environments than it is informative about the presence or absence of strategic partner-building behavior.}
...
\added{\textit{age\_mean} (team average player age at the tournament) is a second natural proxy for career maturity alongside \textit{exp\_mean} (years since turning pro); the two are highly correlated (corr = 0.83, implied VIF $\approx$ 3.30 if entered together), so age is estimated in its own separate specification rather than added alongside experience in the main tables.}
...
\toprule
 & (1) Same Nationality & (2) Same Language & (3) Lang. Proximity \\
\midrule
Cultural similarity & 0.0355** & 0.0467*** & 0.0428*** \\
...
\begin{tablenotes}
\small
\item *** p$<$0.01, ** p$<$0.05. Same sample, FE, and clustering as Table \ref{tab:t3}. Cultural-similarity AMEs are essentially unchanged from Table \ref{tab:t3} (e.g.\ same nationality: 0.0377 $\to$ 0.0355, both p$<$0.05), confirming the core finding is not an artefact of which career-maturity proxy is used.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\toprule
 & (1) Same Nationality & (2) Same Language & (3) Lang. Proximity \\
\midrule
Cultural similarity & 0.0396** & 0.0485*** & 0.0443*** \\
Own avg. doubles ranking & $-$0.0007*** & $-$0.0007*** & $-$0.0007*** \\
 & (0.0001) & (0.0001) & (0.0001) \\
Opponent avg. doubles ranking & 0.0007*** & 0.0007*** & 0.0007*** \\
 & (0.0001) & (0.0001) & (0.0001) \\
...
\begin{tablenotes}
\small
\item Diagnostic only, not a preferred spec. With both tenure proxies in the model, neither linear term remains significant (p$>$0.37), and SEs roughly double relative to the single-proxy specs -- the expected signature of near-collinear regressors (corr = 0.83), not evidence that career maturity stops mattering. The cultural-similarity AMEs remain robust throughout.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\toprule
 & (1) Same Nationality & (2) Same Language & (3) Lang. Proximity \\
\midrule
Cultural similarity & 0.0243 & 0.0036 & $-$0.0020 \\
...
\midrule
Same nationality & 0.0298 (n.s.) & 0.0316~[0.0040, 0.0759] & 1/10 \\
Same language & 0.0075 (n.s.) & 0.0024~[$-$0.0255, 0.0678] & 1/9\tnote{a} \\
Lang.\ proximity & 0.0019 (n.s.) & $-$0.0047~[$-$0.0346, 0.0624] & 1/9\tnote{a} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item[a] One of the 10 tiebreak seeds produced a singular Hessian (non-identified MLE) in 2 of 3 culture specs, due to sparse tournament$\times$year$\times$round cells once the sample is halved ($N\approx$1,182); that draw is excluded rather than reported as spurious.
\item Tiebreak win, $N\approx$1,182 per draw. Every point estimate keeps the same sign and a broadly similar magnitude to baseline (Table \ref{tab:t4}); the stacked-both-rows, match-clustered design used throughout the paper remains the preferred specification.
\end{tablenotes}
\end{threeparttable}
\end{table}
