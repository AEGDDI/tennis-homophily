...
We construct three measures of cultural similarity. The first measure, \textit{Same Nationality}, equals one if both teammates represent the same country and zero otherwise. The second measure, \textit{Same Language}, equals one if both teammates share an official national language and zero otherwise. \textcolor{blue}{We construct this measure from CEPII's Gravity database \citep{contecotterlazmayer2022}\footnote{\added{Centre d'\'etudes prospectives et d'informations internationales (CEPII), \texttt{https://www.cepii.fr/CEPII/en/bdd\_modele/bdd\_modele.asp}.}}, which records, for each pair of countries, whether they share an official language. A small number of country pairs in our sample are not covered by the source table; for these, we manually checked whether the two countries in fact share an official language.} The third measure, \textit{Linguistic Proximity}, is a continuous index on $[0,1]$ built from Ethnologue's language-tree classification \citep{melitztoubal2014}, obtained from the authors' own data repository (\texttt{faridtoubal.com/Paper/LangJIE/lang.html}).
This continuous measure distinguishes teammates who speak closely related but distinct languages (e.g.\ Italian and Spanish, both Romance languages) from teammates whose languages are totally unrelated. It is the same measure used in \citet{bekesottaviano2025}  to characterize linguistic similarity between teammates in football teams.\footnote{\citet{melitztoubal2014} score each pair of \emph{languages} on a coarse ordinal scale according to how many levels of the Ethnologue family tree they share (e.g.\ two languages in the same branch but a different sub-branch score $0.5$), and the resulting \emph{country}-pair value is a population-weighted average across each country's principal languages. For example, Belgium is absent from their table; we then construct its value against any other country as the population-weighted average of the Netherlands' and France's own values against that country, using Belgium's approximate Dutch/French population shares ($59\%$/$41\%$) as weights, the same way Melitz and Toubal (2014) themselves apply to other multi-language countries such as Switzerland. This gives Belgium a linguistic-proximity value of $0.56$ against France ($0.59\times0.25+0.41\times1.00$: Dutch and French sit in different branches of the same language family, while French scores trivially $1.00$ against itself) and $0.69$ against the Netherlands ($0.59\times1.00+0.41\times0.25$).} \textcolor{blue}{Unlike settings in which dual citizenship is common, ATP players compete under a single playing nationality, so each player contributes exactly one country code to this measure and no dual-nationality assignment rule is needed. The language value itself, however, is defined at the country level, from each country's aggregate population-language composition, not at the level of the individual player: a player who is personally bilingual, or whose own mother tongue differs from his playing nationality's majority language, is assigned the value implied by that nationality, with no separate adjustment for his own linguistic background.}\footnote{An alternative binary construction of linguistic proximity (CEPII's \textit{comlang\_ethno}, one if both teammates share a widely-spoken ethnic language and zero otherwise, defined as a language spoken as a mother tongue by at least 9\% of the population in both countries) and two further continuous constructions from \citet{psw2024} are reported as robustness checks in Appendix~\ref{app:langprox}.}
...
In our sample, 42.2\% (882 teams) of teams consist of players sharing the same nationality, 56.7\% (1,184 teams) share an official language, and the mean of the continuous measure linguistic proximity is 0.654.
