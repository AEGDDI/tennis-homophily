...
\textcolor{red}{List 2-3 evidence from the literature: firm, academic, sports. Firm evidence needed!}

\added{In academics, \citet{freemanhuang2015} document that culturally self-sorted homogeneous teams face a distinct quality penalty, receiving fewer citations than diverse teams. %Tonellato (2025) Journal of Management Surgical teams; Dale-Olsen and Finseraas (2019) find higher workforce linguistic diversity decreases productivity in Manufactures.
In sports, \citet{bekesottaviano2025} show that culturally similar players are more likely to pass to one another within elite football teams,  however, such pattern may not necessarily enhance the overall performance.
...
A retrospective by \citet{stahlmaznevski2021} argues that the effect of cultural diversity on team performance is not universal and depends on the nature of the task, highlighting the need for future research.} %Yet whether cultural similarity between collaborators ultimately translates into better team performance, particularly when collaborators themselves have substantial discretion over partner selection, remains less clear.
...
In this paper, we use professional men's doubles tennis as an empirical setup to examine 1) the relationship between cultural similarity and team performance,  2) the boundary condition of this relationship, and 3) the extent to which cultural similarity reflects systematic partner sorting in our setting.\footnote{Partnership formation in professional doubles tennis is inherently multidimensional. Players may sort on several characteristics, including playing ability and experience, strategic complementarity, and institutional or commercial connections such as shared representation by an agency or sponsorship arrangement. Our analysis abstracts from these multiple dimensions of partner selection and focuses on cultural similarity as one potentially consequential dimension of the resulting partnership.} 
...
\iffalse
==================================================================================
NOTE FOR LINGQING (not compiled into the PDF -- delete this block whenever) --
Candidate additional references, already added to reference.bib, in case useful
for the discussion. None of these are cited in the text yet.

- \citet{ertugetal2021} -- review of homophily's *consequences* (not formation),
  structured by level of analysis incl. the dyad -- matches this paper's question.
- \citet{mcphersonetal2001} -- canonical homophily review; good anchor for the
  self-sorting framing, and distinguishes choice homophily from homophily driven
  by a constrained opportunity structure (relevant to the Freeman \& Huang tension
  in footnote 4).
- \citet{claesvissa2020} -- firm-level homophily-to-performance evidence: does
  similarity between two parties (VC and founder) translate into better financial
  outcomes -- same question, different dyad/domain.
- \citet{hjort2014} -- firm-level: ethnically mixed vs. homogeneous production
  teams and output, in a real-production-team setting.
- \citet{munshi2003} -- agents with less market power rely more on same-group
  networks from fewer outside options, not preference -- reconciles the Freeman \&
  Huang tension in footnote 4.
- \citet{ahujaetal2009} -- status shapes who sorts with whom, in a firm-alliance
  setting -- parallel mechanism to the footnote 4 tension, different context.
- \citet{daleolsenfinseraas2020} -- linguistic diversity and workplace
  productivity; already drafted but commented out in your text.
- \citet{huangetal2023jersey} -- cultural diversity and shared team tenure on
  national-team performance; complements the Section 5 experience-interaction
  finding.
- \citet{petruzzelli2019} -- weaker fit; R\&D alliance formation via status-for-
  knowledge trades, no direct link to cultural similarity or dyadic performance.
==================================================================================
\fi
...
In our sample,  \textcolor{red}{42.2\%} of teams consist of players sharing the same nationality, \textcolor{red}{56.0\%} share an official language, and \textcolor{blue}{the mean linguistic proximity (\textit{prox1}, continuous on $[0,1]$) is 0.651}.
...
\added{Alternative specification: randomly assign one team per match as the focal team to be kept in the regression analysis (N=1,864). Coefficients are of similar magnitude and slighter weaker significance.} \textcolor{red}{Repeated over 10 random seeds: the same-nationality AME averages 0.0466 (range 0.0161--0.0812, significant at the 5\% level in 5 of 10 draws), same-language averages 0.0517 (range 0.0230--0.0877, significant in 7 of 10 draws), and language proximity averages 0.0471 (range 0.0156--0.0859, significant in 6 of 10 draws).}
...
\subsection{Robustness: Continuous Linguistic Proximity}

\added{Column (3) of Table~\ref{tab:t3} measures linguistic proximity as CEPII's binary \textit{comlang\_ethno} indicator. As a robustness check, we replace it with three continuous measures, one at a time, keeping every other element of the specification identical: \textit{prox1}, the Ethnologue tree-based linguistic proximity index of \citet{melitztoubal2014} -- the same underlying measure used by \citet{bekesottaviano2025} for their own language-similarity variable -- together with two independently constructed measures from \citet{psw2024}, an inverted tree-distance index and a cognate/lexical-based proximity index, as further robustness. All three are continuous on $[0,1]$ and achieve complete coverage on our sample (no missing country pairs, after the same Monaco fallback rule used for \textit{same\_language}/\textit{ling\_prox}).}

\begin{table}[htbp]
\color{blue}
\centering
\caption{Match Win — Continuous Linguistic Proximity (Robustness) \label{tab:t3d}}
\begin{threeparttable}
\begin{tabular}{lccc}
\toprule
 & (1) prox1 & (2) PSW tree & (3) PSW cognate \\
\midrule
Linguistic proximity & 0.0527*** & 0.0648*** & 0.0719*** \\
 & (0.0201) & (0.0231) & (0.0231) \\
Own avg. doubles ranking & $-$0.0007*** & $-$0.0007*** & $-$0.0007*** \\
 & (0.0001) & (0.0001) & (0.0001) \\
Opponent avg. doubles ranking & 0.0007*** & 0.0007*** & 0.0007*** \\
 & (0.0001) & (0.0001) & (0.0001) \\
Top-100 singles player (0,1)& $-$0.0034 & $-$0.0038 & $-$0.0034 \\
 & (0.0314) & (0.0314) & (0.0315) \\
Team avg. professional years  & 0.0044*** & 0.0046*** & 0.0047*** \\
 & (0.0017) & (0.0017) & (0.0017) \\
Team avg. professional years$^2$  & $-$0.0008*** & $-$0.0008*** & $-$0.0008*** \\
 & (0.0002) & (0.0002) & (0.0002) \\
\midrule
...
\begin{tablenotes}
\small
\item *** p$<$0.01, ** p$<$0.05, * p$<$0.10. Standard errors in parentheses, clustered by match. Same sample, controls, and FE as Table~\ref{tab:t3}; AME reported per full-scale (0$\to$1) change in the linguistic-proximity measure, directly comparable to Table~\ref{tab:t3} column (3)'s binary AME. Per-SD AMEs: prox1 = 0.0206, PSW tree = 0.0225, PSW cognate = 0.0249.
\end{tablenotes}
\end{threeparttable}
\end{table}
\FloatBarrier

\added{The continuous measures agree with the binary specification in Table~\ref{tab:t3}: linguistic proximity remains positive, statistically significant at the 1\% level, and of comparable (if somewhat larger) magnitude across all three continuous constructions, ranging from 5.3 to 7.2 percentage points for a full-scale change in proximity. The result is therefore not an artifact of the binary CEPII threshold rule, nor specific to any one continuous data source.}
...
We estimate the same specification used for the match-level analysis, replacing the match-win outcome with an indicator for winning the tiebreak. Table~\ref{tab:t4} reports the results. Across all three measures of cultural similarity, the estimated associations are substantially smaller than those observed for match outcomes and are not statistically distinguishable from zero. The absence of a detectable association is consistent across shared nationality, shared official language, and linguistic proximity.
...
\textcolor{blue}{With linguistic proximity now measured continuously (\textit{prox1}), the culture $\times$ experience interaction is positive but no longer statistically significant (p=0.211) -- unlike shared language, where the interaction remains significant. The binary CEPII construction of language proximity used prior to 2026-09-10 (\textit{comlang\_ethno}, reported in Appendix~\ref{app:langprox}) did show a significant interaction (p=0.034) for this measure; the two continuous PSW2024 robustness measures in the same Appendix table are also insignificant (p=0.305, p=0.265), agreeing with the main-text \textit{prox1} result rather than the binary one. We therefore read the experience-amplification finding as robust for shared official language, but not for linguistic proximity once measured on a continuous, graded scale rather than a coarse same/different threshold.}

This pattern is consistent with a mechanism based on accumulated professional experience and the development of effective coordination. More experienced players may be more able to anticipate teammates' actions and establish efficient communication routines. Cultural and linguistic similarity may make these coordination processes more effective, so that their association with performance becomes stronger among more experienced teams. \textcolor{blue}{The fact that the amplification result is concentrated in shared official language -- and not in the continuously-measured linguistic proximity -- is consistent with a communication-based interpretation tied specifically to official/formal language use, rather than to graded linguistic closeness more broadly.}
...
\multirow{2}{*}{\textcolor{blue}{Ling.\ proximity (mean)}} & Actual & \textcolor{blue}{0.651} & \textcolor{blue}{0.601} & \textcolor{blue}{0.631} \\
...
\textcolor{blue}{Ling.\ proximity (mean)} & \textcolor{blue}{0.618} & \textcolor{blue}{0.660} & \textcolor{blue}{0.649} & \textcolor{blue}{0.646} & \textcolor{blue}{0.664} & \textcolor{blue}{0.675} \\
\midrule
