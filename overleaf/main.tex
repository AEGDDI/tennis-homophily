...
\textcolor{blue}{We construct three measures of cultural similarity \textcolor{red}{using dataset xxx CEPII (reference)... and cite 1-2 papers using this dataset or measure.}. The first measure, \textit{Same Nationality}, equals one if both teammates represent the same country and zero otherwise. The second measure, \textit{Same Language}, equals one if both teammates share an official national language and zero otherwise.\footnote{\textcolor{red}{Explain what is proxy1 here and  add the link of reference. Nobody knows what is proxy1 if you don't explain it.}} The third measure, \textit{Language Proximity}, is a continuous index on $[0,1]$ built from Ethnologue's language-tree classification \citep{melitztoubal2014}.} \textcolor{red}{Give a concrete example of how it's calculated: for example Belgium and France, or Germany and Netherlands etc. Some players can be bilingual, for example, Alex De Minaur speaks both Spanish and English. Zverev both German and Russian, how is this handeled in the data?}\footnote{An alternative binary construction of language proximity (CEPII's \textit{comlang\_ethno} -- one if both teammates share a widely-spoken ethnic language and zero otherwise, defined as a language spoken as a mother tongue by at least 9\% of the population in both countries) and two further continuous constructions from \citet{psw2024} are reported as robustness checks in Appendix~\ref{app:langprox}.}
...
These measures reflect increasingly broad notions of cultural similarity. Shared nationality captures common institutional and cultural background. Shared official language captures communication potential through the language used in formal and public life. Language proximity recognizes that communication costs may be lower between teammates whose languages are closely related even when they do not share the same official language.  %Language Proximity and Same Language can diverge substantially -- e.g. two Romance-language speakers score a high \textit{prox1} without sharing an official language.

Table 2: \textcolor{red}{I would remove Panel B and add the absolute number of partnerships next to the share in Panel A, 42.2\%=X teams for each of the 3 measure.}  In our sample,  \textcolor{red}{42.2\%} of teams consist of players sharing the same nationality, \textcolor{red}{56.0\%} share an official language, and \textcolor{blue}{the mean of the continuous measure language proximity is 0.654}.
...
Same language  & 56.0\% \\
\textcolor{blue}{Ling.\ proximity (mean, prox1)} & \textcolor{blue}{0.654} \\
\bottomrule
\end{tabular}%
}
&
...
\multirow{2}{*}{\textcolor{blue}{Ling.\ proximity (mean, prox1)}} & Actual & \textcolor{blue}{0.654} & \textcolor{blue}{0.603} & \textcolor{blue}{0.632} \\
...
\textcolor{blue}{Formally, for a focal player from country $c$ entering a tournament-year field of $N$ total entrants (every player who appears in at least one match at that event, not merely the rank-complete partnerships used in Table~\ref{tab:t8}), the nationality field-composition measure is $\text{composition}_{nat}(c) = \#\{\text{other field entrants from country } c\} \, / \, (N-1)$; the language and linguistic-proximity composition measures replace the numerator with the sum, over every other field entrant $j$, of \textit{same\_language}$(c,j)$ or \textit{prox1}$(c,j)$ respectively, again divided by $N-1$. As an illustration, at Roland Garros 2019 the host nation contributed by far the largest national contingent (20 of 128 entrants); an Italian player that year faced a field of 4 (Berrettini, Sonego, Seppi, Cecchinato) total Italians (3 others besides himself) among 127 other entrants, giving $\text{composition}_{nat}=\text{composition}_{lang}=3/127=0.024$, since no other nationality present in that field shared Italian as an official language. The corresponding linguistic-proximity composition is substantially higher, at $0.344$: although almost no entrant shared Italian officially, many were linguistically related to it -- France's 20 entrants alone, each scoring \textit{prox1}$=0.5$ against Italian (both Romance languages, same branch but different sub-branch of the Ethnologue tree), contributed $20\times0.5=10.0$ to the numerator, roughly a quarter of that player's entire composition score that year.}

The results are reported in Table~\ref{tab:t9}. In Spec 1 (own ranking alone), the coefficient on own ranking is positive and statistically significant for all three measures: 0.000269 for same nationality (p<0.001), 0.000146 for same language (p=0.012), and \textcolor{blue}{0.000107 for linguistic proximity (p=0.012; OLS coefficient rather than AME, since \textit{prox1} is continuous)}. Because a lower doubles-ranking number indicates a stronger player, the positive coefficient means that worse-ranked players are, if anything, more likely to have culturally similar partners.
...
\multirow{4}{*}{Same language (AME)}
 ...
 & & & (0.081709) & (0.082578) \\
\addlinespace
\multirow{4}{*}{\textcolor{blue}{Ling.\ proximity (OLS coef., prox1)}}
...
\textcolor{blue}{Ling.\ proximity (mean, prox1)} & \textcolor{blue}{0.619} & \textcolor{blue}{0.660} & \textcolor{blue}{0.651} & \textcolor{blue}{0.649} & \textcolor{blue}{0.664} & \textcolor{blue}{0.683} \\
\midrule
...
The share of same-nationality teams is remarkably stable across the Tokyo cycle's three periods, at 40.0\%, 40.3\%, and 39.0\%, respectively (Table~\ref{tab:t2ab}). The corresponding shares for shared official language are 54.8\%, 55.0\%, and 54.8\%, while \textcolor{blue}{mean linguistic proximity (\textit{prox1}) is 0.619, 0.660, and 0.651}.
...
\added{The main text measures linguistic proximity with \textit{prox1} \citep{melitztoubal2014}, a continuous $[0,1]$ index built from Ethnologue's language-tree classification (the same measure used by \citet{bekesottaviano2025}). This Appendix reports every main-text regression table (3, 4, 5, 6, 6b) re-estimated with three alternative constructions of linguistic proximity, one at a time, in place of \textit{prox1}: (i) the original binary CEPII \textit{comlang\_ethno} indicator; and two further continuous, independently-constructed robustness measures from \citet{psw2024}, (ii) an inverted tree-distance index and (iii) a cognate/lexical-based proximity index. All three achieve complete coverage on our sample. Every table below uses the identical sample, controls, and fixed effects as its main-text counterpart.}

\begin{table}[htbp]
\centering
\caption{Appendix A3 -- Match Win, Language Proximity Robustness (cf.\ Table~\ref{tab:t3}) \label{tab:a3}}
\begin{threeparttable}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[htbp]
\centering
\caption{Appendix A4 -- Tiebreak Win, Language Proximity Robustness (cf.\ Table~\ref{tab:t4}) \label{tab:a4}}
\begin{threeparttable}
\begin{tabular}{lccc}
\toprule
 & (1) Binary (comlang\_ethno) & (2) PSW tree & (3) PSW cognate \\
...
\begin{table}[htbp]
\centering
\caption{Appendix A5 -- Culture $\times$ Surface, Language Proximity Robustness (cf.\ Table~\ref{tab:t5}) \label{tab:a5}}
\begin{threeparttable}
\begin{tabular}{lcccccc}
\toprule
...
\begin{table}[htbp]
\centering
\caption{Appendix A6 -- Culture $\times$ Years Since Turning Pro, Language Proximity Robustness (cf.\ Table~\ref{tab:t6}) \label{tab:a6}}
\begin{threeparttable}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[htbp]
\centering
\caption{Appendix A6b -- Culture $\times$ Within-Team Experience Gap, Language Proximity Robustness (cf.\ Table~\ref{tab:t6b}) \label{tab:a6b}}
\begin{threeparttable}
\begin{tabular}{lccc}
\toprule
 & (1) Binary & (2) PSW tree & (3) PSW cognate \\
...
\added{Across every table, the binary CEPII measure and the two PSW2024 continuous measures agree qualitatively with the main-text \textit{prox1} results, with one exception noted in Section~5: the binary measure's culture $\times$ experience interaction (Appendix Table~\ref{tab:a6}, column 1) is statistically significant, while the main-text \textit{prox1} interaction and both PSW2024 robustness interactions are not.}
...
\textcolor{blue}{The randomly-selected-team specification produces AMEs of similar magnitude to Table~\ref{tab:t3}'s main estimates for all three measures, with somewhat weaker and less stable significance -- expected given the sample is halved and no longer benefits from the efficiency gain of clustering two related observations per match.\\}
